...
namespace wServer.realm
{
    public class StatsManager
    {
        ...
        //so can't we make a kk
        public float GetAttackDamage(int min, int max)
        {
            return Random.obf6((uint)min, (uint)max) * DamageModifier();
        }
        public int GetHpSomething(int amount)
        {
            return amount + HPBOOSTSTUFF();
        }
        public int HPBOOSTSTUFF()
        {
            return (player.Boost[3]);
        }
        public int GetATTTKSomething(int amount)
        {
            return amount + ATTKBOOSTSTUFF();
        }
        public int ATTKBOOSTSTUFF()
        {
            return (GetStats(2) / 5);
        }
        ...
        private float DamageModifier()
        {
            ...
            if (player.HasConditionEffect(ConditionEffectIndex.Damaging))
                ret *= 1.5f;
            if (player.HasConditionEffect(ConditionEffectIndex.Weak) && player.HasConditionEffect(ConditionEffectIndex.Damaging))
                ret *= 1.0f;
            return ret;
        }
        ...
        public static float GetDefenseDamage(Entity host, int dmg, int def)
        {
            ...
            if (host.HasConditionEffect(ConditionEffectIndex.ArmorBroken))
                def = 0;
            if (host.HasConditionEffect(ConditionEffectIndex.ArmorBroken) && host.HasConditionEffect(ConditionEffectIndex.Armored))
                def *= 1;
        ...
        public float GetDefenseDamage(int dmg, bool noDef)
        {
            ...
            if (player.HasConditionEffect(ConditionEffectIndex.ArmorBroken) ||
                noDef)
                def = 0;
            if (player.HasConditionEffect(ConditionEffectIndex.ArmorBroken) && player.HasConditionEffect(ConditionEffectIndex.Armored))
                def *= 1;
        ...
        {
            int vit = GetStats(5);//couldn't hear where u wanted me to go back up 2
